...
export default function BootSequence({ onComplete }) {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('netwatch_boot_seen')) {
      onComplete();
      return;
    }
  ...
  const handleSkip = () => {
    ...
    sessionStorage.setItem('netwatch_boot_seen', '1');
    onComplete();
  };
          ...
          if (okMatch) {
            return (
              <div key={i} style={{ lineHeight: '1.5' }}>
                <span style={{ color: '#00ff41' }}>{okMatch[1]}</span>
                <span style={{ color: msg.color }}>{okMatch[2]}</span>
              </div>
            );
          }

          return (
            <div key={i} style={{ color: msg.color, lineHeight: '1.5' }}>
              {msg.text}
            </div>
          );
        })}
