...
const COMPILE_STEPS = [
  ...
  { text: '> Verifying on Monad Network...', delay: 2700 },
];
...
export default function CompilingOverlay({ success, onComplete }) {
  ...
  const color = success ? '#30FF60' : '#ff3333';

  return (
    <div className="ms-compiling-overlay">
      ...
      <div style={{ fontFamily: '"Courier New", monospace', fontSize: '12px', textAlign: 'left', minWidth: '300px' }}>
        {lines.map((line, i) => (
          <div key={i} style={{
            color: line.includes('FAILED') || line.includes('Errors') ? '#ff3333' : line.includes('OK') ? '#30FF60' : '#aaa',
          }}>
            {line}
          </div>
        ))}
