...
function analyzeContract(code) {
  ...
  if (pragmaMatch) {
    ...
    if (version.includes('0.8')) {
      findings.push({ severity: 'info', title: 'Solidity Version', desc: `Detected: ${version}. Recommended: ^0.8.28 for Monad.` });
    } else {
      findings.push({ severity: 'warning', title: 'Outdated Solidity', desc: `Detected: ${version}. Upgrade to ^0.8.28 for Monad compatibility.` });
      score -= 10;
    }
  }
  ...
  if (!code.includes('prague')) {
    findings.push({ severity: 'error', title: 'Missing evmVersion', desc: 'Must set evmVersion: "prague" in compiler settings for Monad.' });
    score -= 5;
  }
  ...
  if (storageReads > 3) {
    findings.push({
      ...
      desc: `Found ${storageReads} mappings. Cold SLOAD costs 8,100 gas on Monad (vs 2,100 on Ethereum). Cache reads in memory variables.`,
    });
    score -= 3;
  }
  ...
  if (externalCalls > 0) {
    findings.push({
      ...
      desc: `Found ${externalCalls} external call(s). Cold account access costs 10,100 gas on Monad. Ensure re-entrancy guards are in place.`,
    });
  }
  ...
  if (!code.includes('unchecked')) {
    findings.push({
      ...
      desc: 'Consider using unchecked blocks for safe arithmetic to save gas on Monad.',
    });
  }
  ...
  findings.push({
    ...
    desc: `~${sizeKB.toFixed(1)} KB source. Monad allows up to 128 KB (vs 24.5 KB on Ethereum).`,
  });
  ...
  if (code.includes('@openzeppelin')) {
    findings.push({
      ...
      desc: 'OpenZeppelin contracts are fully compatible with Monad. Ensure you use v5.x imports.',
    });
  }
      ...
      <div className="mb-flex mb-items-center mb-gap-lg mb-mb-lg">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div className="mb-text-sm">
            {score >= 90
              ? 'Your contract is nearly ready for Monad. Review the notes below.'
              : 'Review the findings below and apply optimizations for Monad.'}
          </div>
        </div>
      </div>
