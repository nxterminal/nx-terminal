...
export default function BootSequence({ onComplete }) {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('parallax_boot_seen')) {
      onComplete();
      return;
    }
  ...
  const handleSkip = () => {
    ...
    sessionStorage.setItem('parallax_boot_seen', '1');
    onComplete();
  };
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        {lines.map((msg, i) => {
          ...
          const isHighlight = msg.text.includes('PARALLEL EXECUTION') || msg.text.includes('IT IS THE FUTURE');
          ...
          if (okMatch) {
            return (
              <div key={i} style={{ lineHeight: '1.5' }}>
                ...
                <span style={{ color: '#30FF60' }}>OK</span>
              </div>
            );
          }
          ...
          return (
            <div key={i} style={{
              color: msg.color,
              lineHeight: '1.5',
              textShadow: isHighlight ? `0 0 8px ${COLORS.primary}` : 'none',
