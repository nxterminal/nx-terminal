...
export default function DevCamp() {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: T.bg, color: T.text, fontFamily: "'VT323', monospace", fontSize: 13 }}>
      {/* Header */}
      <div style={{ flexShrink: 0, padding: '8px 12px', borderBottom: `1px solid ${T.cardBorder}` }}>
        <div style={{ fontWeight: 'bold', fontSize: 18, color: T.cyan }}>{'>'} DEV CAMP</div>
        <div style={{ fontSize: 12, color: T.textDim, marginTop: 2 }}>Train your devs. Permanently boost stats. Unlock harder missions.</div>
      </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', borderBottom: `1px solid ${T.cardBorder}` }}>
        {[['catalog', 'Catalog'], ['training', `In Training${trainingDevs.length ? ` (${trainingDevs.length})` : ''}`]].map(([k, label]) => (
          <button key={k} onClick={() => setTab(k)} style={{
            fontSize: 13, padding: '5px 14px', fontWeight: tab === k ? 'bold' : 'normal',
      ...
      {/* Feedback */}
      {feedback && (
        <div style={{ flexShrink: 0, fontSize: 13, fontWeight: 'bold', padding: '6px 12px', margin: '4px 10px 0', border: `1px solid ${feedback.color}`, background: `${feedback.color}15`, color: feedback.color }}>{feedback.text}</div>
      )}
        ...
        {tab === 'catalog' && (
          <>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 4 }}>SELECT DEV TO ENROLL:</div>
              <select value={selectedDev} onChange={e => setSelectedDev(e.target.value)} style={{
                fontFamily: "'VT323', monospace", fontSize: 13, background: T.card, color: T.green,
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: '#ffaa00', fontWeight: 'bold', marginBottom: 4, borderBottom: `1px solid ${T.cardBorder}`, paddingBottom: 3 }}>
            ...
            {/* Courses */}
            <div>
              <div style={{ fontSize: 13, color: '#ff6644', fontWeight: 'bold', marginBottom: 4, borderBottom: `1px solid ${T.cardBorder}`, paddingBottom: 3 }}>
        ...
        {tab === 'training' && (
          trainingDevs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: T.textDim }}>
              <div style={{ fontSize: 16, marginBottom: 8 }}>No devs in training</div>
              <div style={{ fontSize: 13 }}>Go to Catalog to enroll a dev.</div>
            </div>
          ) : trainingDevs.map(d => (
            <div key={d.token_id} style={{ padding: '10px 12px', marginBottom: 6, background: T.card, border: `1px solid ${d.can_graduate ? T.green + '60' : T.cardBorder}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, color: T.text, fontWeight: 'bold' }}>{d.name} <span style={{ color: T.textDim, fontWeight: 'normal' }}>[{d.archetype}]</span></div>
                  <div style={{ fontSize: 12, color: T.textMuted }}>{d.training_name} — +{d.boost} {d.stat?.toUpperCase()}</div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, color: d.can_graduate ? T.green : T.textDim, minWidth: 30 }}>{d.progress}%</span>
              </div>
              <div style={{ marginTop: 6, textAlign: 'right' }}>
                {d.can_graduate ? (
                  <button className="win-btn" onClick={() => handleGraduate(d.token_id)} disabled={busy} style={{
                    fontSize: 13, padding: '5px 14px', fontWeight: 'bold',
  ...
  return (
    <div style={{
      ...
      <button className="win-btn" disabled={disabled} onClick={onBuy} style={{
        fontSize: 12, padding: '4px 12px', fontFamily: "'VT323', monospace",
